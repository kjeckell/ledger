...
def add_player(event, context):
    ...
    try:
        ...
        if e == 'ConditionalCheckFailedException':
            ...
            retVal['body'] = json.dumps(
                {'message': 'Email already exists', 'success': False, 'player': {}})
            return retVal
        else:
        ...
        # Adding the 'body' key and contents for delivery back to requestor
        retVal['body'] = json.dumps(
            {'message': 'Player Added', 'success': True, 'player': player})
        return retVal
    ...
    if (event["queryStringParameters"] is not None):
        ...
        if 'email' in qsParams:
            ...
            try:
                ...
                retVal['body'] = json.dumps(
                    {'message': 'No Player Found', 'success': False, 'player': {}})
                return retVal
            else:
                ...
                retVal['body'] = json.dumps(
                    {'message': 'Player Returned', 'success': True, 'player': player})
                return retVal
...
def update_player(event, context):
    ...
    if 'email' in payLoad:
        ...
        try:
            ...
            print(e)
            retVal['body'] = json.dumps(
                {'message': 'Player Not Found', 'success': False, 'player': {}})
            return retVal
        else:
            print(payLoad['email'] + ' was found')
            # taking the existing player object in from the initial search
            player = response['Item']
            ...
            try:
                ...
                retVal['body'] = json.dumps(
                    {'message': 'Player Updated', 'success': True, 'player': player})
                return retVal
    # We never got an email in the payload so we cant update anything
    else:
        retVal['body'] = json.dumps(
            {'message': 'No Email Recieved', 'success': False, 'player': {}})
        return retVal
